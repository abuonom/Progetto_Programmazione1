...
3 Controllare se è possibile soddisfare la richiesta
...
6 Dato un tipo di carburante visualizzare il giorno in cui si è avuta la massima vendita. 
...
3) Rifornimento del veicolo, laddove la scorta lo consente, altrimenti il distributore dÃ  indietro i soldi
all'utente.
4) Visualizza in quale giorno della settimana si è venduto più carburante.
Con qualsiasi altro numero do-while resta dentro fino a quando la variabile menu è compresa tra 1 e 4
...
e controlla se il numero dei litri Ã¨ inferiore a 10, se si, rifornisce la scorta di carburante.*/
void in_soldi(double *soldi);/*procedura che inserisce i soldi nel distributore, accetta 5,10,20 o 50â‚¬
...
int main(){
    ...
    int massimo; //variabile utilizzata per trovare il giorno della settimana in cui si e' venduto piÃ¹ carburante
    int menu;//variabile utilizzata per l'input dell'utente del menÃ¹
    ...
    do{
            ...
            /*Se menù=1 allora viene richiesto all'utente quale tipo di carburante vuole settare
            ...
            if(menu == 3){
                if(tipo_carburante == 1){
                    rifornimento(&benzina,&giorni_b[giorno],&soldi);/*Rifornisco benzina se tipo_carburante Ã¨ 1
                ...
                if(tipo_carburante == 2){
                    rifornimento(&metano,&giorni_m[giorno],&soldi);/*Rifornisco metano se tipo_carburante Ã¨ 2
                    Passo l'indirizzo della struttura Carburante, l'indirizzo della variabile giorni_benzina con l'indice
                    giorno (che indica il giorno della settimana in cui ci troviamo) e l'indirizzo della variabile soldi*/
                    printf("\n");
                    controllo_periodico++;//incremento di 1 controllo periodico
                ...
                if(tipo_carburante == 3){
                    rifornimento(&gpl,&giorni_g[giorno],&soldi);/*Rifornisco gpl se tipo_carburante Ã¨ 3
                    Passo l'indirizzo della struttura Carburante, l'indirizzo della variabile giorni_benzina con l'indice
                    giorno (che indica il giorno della settimana in cui ci troviamo) e l'indirizzo della variabile soldi*/
                    printf("\n");
                    controllo_periodico++;//incremento di 1 controllo periodico
            ...
            if(menu == 4){
                ...
                indice_massimo.
                Indice_max non Ã¨ altro che una function che ritorna la posizione del valore piÃ¹ alto tra gli elementi
                ...
                Servendomi dell'indice stampo il giorno in cui si e' venduto di piÃ¹ ed il valore in euro del carburante erogato*/
                if(tipo_carburante == 1){
                ...
                if(tipo_carburante == 0){
                    ...
                    /*Se l'utente non ha ancora inserito il tipo di carburante, prima di richiedere
                      in quale giorno si Ã¨ venduto piÃ¹ carburante, allora stampo un errore*/
                }
            }

            /*Se controllo_periodico è 5 allora richiamo la procedura che controlla le scorte di carburante
...
Se il puntatore di soldi è 0, allora stampa errore e ritorna al menù, altrimenti:
...
/*Con indice_max che prende in input un vettore di interi, cerco nel vettore l'indice dell'elemento col valore piÃ¹ alto
